...
from database.solrdb import post_search_in_solr
from helpers.env_constants import get_solr_base_uri_env
from models.text_embeddings import generate_text_embeddings
import asyncio

async def search_item_name_with_vectors_general(text_query, lat, lon, radius, page, rows_per_page, sorting_value, filters):
    sorting_dict = {
        "RELEVANCE": None,
        "DISTANCE_HIGH_TO_LOW": 'geodist() desc',
        "DISTANCE_LOW_TO_HIGH": 'geodist() asc',
        "PRICE_HIGH_TO_LOW": 'item_selling_price desc',
        "PRICE_LOW_TO_HIGH": 'item_selling_price asc'
    }
    ...
    vector_limit = 1000
    params = {
        "q": f"item_name:{text_query}^3"
             f" OR item_short_desc:{text_query}^2 OR item_long_desc:{text_query}^1 " +
             f"OR {{!knn f=item_name_vector topK={vector_limit}}}{text_query_vector}^3 " +
             f"OR {{!knn f=item_short_desc_vector topK={vector_limit}}}{text_query_vector}^2 " +
             f"OR {{!knn f=item_long_desc_vector topK={vector_limit}}}{text_query_vector}^1" +
             f"AND {{!geofilt sfield=provider_geo pt={lat},{lon} d={radius}}}",
        "fq": filter_query,
        "fl": "id,domain,bpp_id,city,item_id,item_currency,item_measure_quantity,item_measure_value,item_name,"
              "item_short_desc,item_long_desc,item_selling_price,item_mrp_price,item_discount_price,item_status,"
              "item_symbol,item_available_count,item_maximum_count,provider_name,provider_status,provider_geo,"
              "provider_id,provider_geo,provider_location_city,provider_location_area_code,provider_location_street,"
              "item_veg,distance:geodist()",
    ...
    solr_url = get_solr_base_uri_env() + "/select"
    results = await post_search_in_solr(solr_url, params)
    print("solr results", results)
    final_response = {
        "grouped": results['grouped'],
        "filters": results['facet_counts']
    }
    return final_response

async def search_item_name_with_vectors(text_query, lat, lon, radius, page, rows_per_page, sorting_value, filters):
    sorting_dict = {
        "RELEVANCE": None,
        "DISTANCE_HIGH_TO_LOW": 'geodist() desc',
        "DISTANCE_LOW_TO_HIGH": 'geodist() asc',
    ...
    # filter_query=[]
    filter_query = [
        f"{{!geofilt sfield=provider_geo pt={lat},{lon} d = {radius}}}"
    ]
    if filters['provider_status_filter'] is not None:
        filter_query.append(f"provider_status:{filters['provider_status_filter']}")
    if filters['item_status_filter'] is not None:
...
async def search_item_name_string_with_vectors(text_query, lat, lon, radius, page, rows_per_page, sorting_value, filters):
    sorting_dict = {
        "RELEVANCE": None,
        "DISTANCE_HIGH_TO_LOW": 'geodist() desc',
        "DISTANCE_LOW_TO_HIGH": 'geodist() asc',
...
async def search_item_name_suggester(text_query,lat,lon,radius):
    try:
        filter_query=[]
        ...
        params = {
            "defType": "edismax",
            "q": f'item_name:("{text_query}"^20) OR item_name_suggest:("{text_query}*"^10 OR "{text_query}"^5 OR "{text_query}"~1)',
            "fl": "id,suggest:item_name_suggest,text:item_name,payload:item_name_suggester_payload_string",
            "fq":filter_query,
            "hl": "true",
            "hl.fl": "item_name_suggest",
            "hl.simple.pre": "<b>",
            "hl.simple.post": "</b>",
            "group": "true",
            "group.field": "item_name_suggest",
            "group.limit": "1"
        }
        if lat and lon and radius is not None:
...
async def search_provider_name_suggester(text_query,lat,lon,radius):
    try:
        filter_query = []
        if lat and lon and radius is not None:
            filter_query.append(f"{{!geofilt sfield=provider_geo pt={lat},{lon} d = {radius}}}")
        params = {
            "defType": "edismax",
            "q": f'item_name:("{text_query}"^20) OR provider_name_suggest:("{text_query}*"^10 OR "{text_query}"^5 OR "{text_query}"~1)',
            "fl": "id,suggest:provider_name_suggest,text:provider_name,payload:provider_name_suggester_payload_string",
            "fq": filter_query,
            "hl": "true",
            "hl.fl": "provider_name_suggest",
            "hl.simple.pre": "<b>",
            "hl.simple.post": "</b>",
            "group": "true",
            "group.field": "provider_name_suggest",
            "group.limit": "1"
        }
        if lat and lon and radius is not None:
            params["sfield"]= "provider_geo"
...
async def search_provider_item_name_suggester(text_query,lat,lon,radius):
    # params = {
    #     "suggest.dictionary": ["itemNameSuggester","providerNameSuggester"],
    #     "suggest.q": f"{text_query}"
    # }
    # solr_url = get_solr_base_uri_env() + "/item_provider_suggester"
    # results = await post_search_in_solr(solr_url, params)
    # print("solr results", results)
    # return results['suggest']
    item_suggest,provider_suggest = await asyncio.gather(
        search_item_name_suggester(text_query,lat,lon,radius),
        search_provider_name_suggester(text_query,lat,lon,radius)
    )
    final_response={
        "item_suggest":item_suggest["response"],
        "provider_suggest":provider_suggest["response"]
    }
    return final_response

async def search_item_name_spell_checker(text_query):
    params = {
        "q": f"{text_query}"
    }
    solr_url = get_solr_base_uri_env() + "/item_name_spell"
    results = await post_search_in_solr(solr_url, params)
    print("solr results", results)
    return results['spellcheck']

async def search_item_name_ss(text_query):
    try:
        params = {
            "suggest": "true",
            "suggest.dictionary": "itemNameSuggester",
            "suggest.q": f"{text_query}",
            "q": f"{text_query}"
        }
        solr_url = get_solr_base_uri_env() + "/item_name_ss"
        results = await post_search_in_solr(solr_url, params)
        print("solr results", results)
        final_response = {
...
if __name__ == '__main__':
    result = asyncio.run(search_item_name_with_vectors("sunfeast", 13.7749, 77.4194, 100, 1, 10, "DISTANCE_HIGH_TO_LOW", {}))
    print(result)
