...
from app.services.solr.solr_service import add_to_mongo, add_to_mongo_processed_document, process_document
...
async def index_documents(body: RequestInitialIndexDb) -> None:
    ...
    while True:
        ...
        if mul:
            ...
            await add_to_mongo_processed_document(processed_docs)
        else:
            final_docs: List[MongoValidDocsType] = [{"collection_type": collection_type, "doc": doc} for doc in valid_docs]
            await asyncio.gather(*(add_to_mongo(doc) for doc in final_docs))
        skip += batch
